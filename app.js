  ...
  socket.on('join', function(sessionID){
    socket.set('sessionID', sessionID, function(){
      if(socket.join(sessionID)) {
          ...
          _to[sessionID].players[socket.id] = { score: 0,acc:0};
          // emit player data
  ...
  socket.on('song-start', function(data){
    socket.get('sessionID', function(err, sessionID){
      if (err) {
        ...
        //io.sockets.in(sessionID).emit('start', data);
        ...
        socket.on('twerk', function(data){
          socket.get('sessionID', function(err, sessionID){
        	...
        	socket.broadcast.emit('player-update',{'id': socket.id, 'score':_to[sessionID].players[socket.id].score,'acc':_to[sessionID].players[socket.id].acc});
            } else {
              console.log("No sessionID");
            }
          });
  ...
  socket.on('song-end', function(data){
    socket.get('sessionID', function(err, sessionID){
      if (err) {
        ...
        io.sockets.in(sessionID).emit("player-data", _to[sessionID].players);
        io.sockets.in(sessionID).emit('end', data);
      } else {
        console.log("No sessionID");
      }
    });
